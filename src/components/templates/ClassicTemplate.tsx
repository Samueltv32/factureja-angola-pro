...
const ClassicTemplate: React.FC<ClassicTemplateProps> = ({
  ...
  return <div className={`a4-content bg-white text-gray-900 font-inter ${className}`}>
      {/* Header */}
        ...
        {data.currentPage === 1 ? (
          ...
          <div className="flex justify-between items-start">
            <div className="flex-1">
              {data.companyLogo && <div className="mb-3">
                  <img src={data.companyLogo} alt="Logo" className="h-12 w-auto object-contain" />
                </div>}
            ...
            <div className="text-right">
              ...
              <div className="text-xs space-y-1">
                ...
                {data.dueDate && <p><span className="font-semibold">Vencimento:</span> {formatDate(data.dueDate)}</p>}
          ...
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-4">
              {data.companyLogo && <img src={data.companyLogo} alt="Logo" className="h-8 w-auto object-contain" />}
              <div>
                <h1 className="text-lg font-bold text-slate-700">{data.companyName}</h1>
                <p className="text-xs text-gray-600">Cliente: {data.clientName}</p>
              </div>
            </div>
            <div className="text-right text-xs">
              <p><span className="font-semibold">Fatura:</span> {data.invoiceNumber}</p>
              <p><span className="font-semibold">Página:</span> {data.currentPage} de {data.totalPages}</p>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 flex flex-col mb-4">
        <div className="flex-1 flex flex-col">
          <table className="w-full border-collapse border border-gray-300">
            ...
            <tbody>
              {data.items.map((item, index) => <tr key={item.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
              ...
              {data.items.length < 20 && Array.from({ length: 20 - data.items.length }).map((_, index) => (
                <tr key={`empty-${index}`} className={(data.items.length + index) % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
        ...
        {showTotal && <div className="flex justify-end mt-3">
      ...
      {showTotal && <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
            ...
            {data.bankDetails && <div className="mt-1 text-xs text-gray-600">
